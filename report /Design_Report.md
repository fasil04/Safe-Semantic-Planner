...
uint64_t id; std::vector embedding; The embedding is used for Euclidean distance calculations and the heuristic function.

Transition Representation
Each transition contains:

-Transition ID -Source state -Destination state -Cost -Safety -Reliability -Availability
...
-vector for states and transitions. -unordered_map for g and rhs values. -Adjacency lists for incoming and outgoing transitions. -Priority queue for the D* Lite OPEN list. -unordered_set for visited-state tracking.
...
-Transition cost -Safety -Reliability -Distance from bad states

The general form is:

Effective Cost = Transition Cost
...
Test Cases -Test Case 1 – Basic Reachability S -> A -> B -> G
The planner should successfully reach the goal.

-Test Case 2 – Bad State Avoidance S -> A -> X -> G
...
-Success -Bad states visited -Total path cost -Minimum safety distance -Safety score -Explored states -Planning time -Replanning time -Approximate memory usage
...
V = number of states E = number of transitions
